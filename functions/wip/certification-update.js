...
pool.on('acquire', function (connection) {
    console.debug('certification-update.js - Connection %d acquired', connection.threadId);
});

pool.on('release', function (connection) {
    console.debug('certification-update.js - Connection %d released', connection.threadId);
});

pool.on('enqueue', function () {
    console.error('certification-update.js - Waiting for available connection slot');
});
    ...
    return new Promise((resolve, reject) => {
        ...
        async function gsrun(cl) {
            ...
            pool.getConnection(function (error, connection) {
                if (error) {
                    reject(error);
                    return console.error(`certification-update.js certificationUpdate() ❌ MySQL login error: '${error.code}'`);
                }
                ...
                connection.query('TRUNCATE club_roster', function (error, results, fields) {
                    if (error) {
                        connection.release();
                        reject(error);
                        return console.error(`certification-update.js certificationUpdate() ❌ TRUNCATE query error: '${error.code}'`);
                    }
                    ...
                    connection.query('INSERT INTO club_roster (guildDiscordID, guildName, guildJoinworld, guildDescription, guildRequirements, guildDiscordLink, guildRepresentative) VALUES ?', [JSONobj], function (error, results, fields) {
                        if (error) {
                            connection.release();
                            reject(error);
                            return console.error(`certification-update.js certificationUpdate() ❌ INSERT query error: '${error.code}'`);
                        }
