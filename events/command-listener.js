        ...
        switch (cmdFile.help.type?.toLowerCase()) {
            case 'botowner': { // only me
                if (channel.type != "dm") {
                    ...
                    else return botReply(`> ${author} Insufficient permissions!\nOnly the bot owner/director can use **${config.botDetails.prefix}${cmdFile.help.name}** command!`, message);
                } else return botReply(`> ${author} **${config.botDetails.prefix}${cmdFile.help.name}** is not available on DM!`, message);
            }
            case "administrator": { // TEA Main Server user with ADMINISTRATOR permission
                if (channel.type != "dm") {
                    ...
                    else return botReply(`> ${author} You don't have access to run **${config.botDetails.prefix}${cmdFile.help.name}**!`, message);
                } else return botReply(`> ${author} **${config.botDetails.prefix}${cmdFile.help.name}** is not available on DM!`, message);
            }
            ...
            else return botReply(`> ${author} **${config.botDetails.prefix}${cmdFile.help.name}** is only available via direct message.`, message);
            ...
            else return botReply(`> ${author} **${config.botDetails.prefix}${cmdFile.help.name}** is currently **disabled**!`, message);

            default: {
                botReply(`> ${author} **${config.botDetails.prefix}${cmdFile.help.name}** Command error, try again later!`, message);
