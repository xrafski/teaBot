...
module.exports = async () => {
	logger.startup(`Handler/Command.js (1) Loaded '${__filename.split('\\').slice(-2).join('/')}' Handler to the.`);

	// Emitted when Mongoose starts making its initial connection to the MongoDB server
	mongoose.connection.on('connecting', () =>
		logger.mongo('Handler/Mongoose.js (2) Connecting to the database.')
	);
	...
	mongoose.connection.on('connected', () =>
		logger.mongo('Handler/Mongoose.js (3) Connected to the database.')
	);

	// Your app called Connection#close() to disconnect from MongoDB
	mongoose.connection.on('disconnecting', () =>
		logger.mongo('Handler/Mongoose.js (4) Disconnecting from the database.')
	);
	...
	mongoose.connection.on('disconnected', () =>
		logger.mongo('Handler/Mongoose.js (5) Disconnected from the database.')
	);
	...
	mongoose.connection.on('close', () =>
		logger.mongo('Handler/Mongoose.js (6) Connection closed.')
	);
	...
	mongoose.connection.on('reconnected', () =>
		logger.mongo('Handler/Mongoose.js (7) Connection restored.')
	);
