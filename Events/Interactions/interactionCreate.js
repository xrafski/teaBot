    ...
    */
    async execute(client, interaction) {
        if (interaction.isCommand() || interaction.isContextMenu()) {
            await interaction.deferReply({ ephemeral: ephemeralToggle(interaction.commandName) }).catch(error => logger.error('Events/Interactions/interactionCreate.js (1) Error to send deferReply', error));
